  ...
  const addOccupant = async (roomNumber) => {
    try {
      const res = await axiosInstance.post(
        `/rooms/${roomNumber}/addOccupant`,
        form
      );
      // Update room in the state with the new occupant
      setRooms(
        rooms.map((room) => (room.roomNumber === roomNumber ? res.data : room))
      );
      setForm({
        name: "",
        role: "",
      ...
      setSuccess("Occupant added successfully.");
  ...
  const deleteOccupant = async (roomNumber, occupantId) => {
    try {
      await axiosInstance.delete(
        `/rooms/${roomNumber}/occupants/${occupantId}`
      );
        ...
        rooms.map((room) =>
          room.roomNumber === roomNumber
            ? {
                ...
                genderAssigned:
                  room.occupants.length === 0 ? null : room.genderAssigned,
              }
            : room
        )
      );
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
          <div className="bg-white p-6 rounded">
            <h2 className="text-xl mb-4">
              Add Occupant to Room {selectedRoom}
            </h2>
            <form
              onSubmit={(e) => {
                e.preventDefault();
